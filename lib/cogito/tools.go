...
func decisionWithStreaming(ctx context.Context, llm LLM, conversation []openai.ChatCompletionMessage,
	...
	for attempts := 0; attempts < maxRetries; attempts++ {
		...
		if len(toolCalls) == 0 {
			if forceTool != "" {
				lastErr = fmt.Errorf("streaming returned no tool calls for forced tool %q (content: %q)", forceTool, content)
				xlog.Warn("[decisionWithStreaming] Forced tool produced no tool call, retrying", "forceTool", forceTool, "attempt", attempts+1)
				time.Sleep(time.Duration(attempts+1) * time.Second)
				continue
			}
...
func toolSelection(llm LLM, f Fragment, tools Tools, guidelines Guidelines, toolPrompts []openai.ChatCompletionMessage, opts ...Option) (Fragment, []*ToolChoice, bool, string, error) {
	...
	if len(selectedTools) == 0 {
		...
		if o.sinkState && results.message != "" {
			...
			resultsMsg := ""
			if results != nil {
				resultsMsg = results.message
			}
			xlog.Debug("[toolSelection] No tool selected but LLM replied (sink state equivalent)", "message", resultsMsg)
			if o.reasoningCallback != nil {
				o.reasoningCallback(reasoning)
			}
			return f, nil, true, resultsMsg, nil
		}
